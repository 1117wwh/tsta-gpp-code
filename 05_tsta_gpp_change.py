...
global vegetated land, water-limited (AI < 1) and energy-limited (AI >= 1)
regions, together with linear trends and the 2023 total in Pg C. This reproduces
the data underlying Fig. 5a,b. (The regional extreme-event panels, Fig. 5c-g, are
simple spatial subsets of the same per-year maps and are not reproduced here.)
...
Inputs :
  ...
  - aridity index raster                  (TerraClimate AI, for the water/energy split)
Outputs:
  - {OUT_DIR}/TSTA_driven_GPP_change_{year}.tif   (per-year maps)
  - {OUT_DIR}/tsta_gpp_change_timeseries.csv       (global/water-/energy-limited series)
"""
...
# Aridity index (TerraClimate) for the water-/energy-limited split:
...
def main():
    ...
    # Water-limited / energy-limited split from the aridity index.
    ai = reproject_to(AI_PATH, ref_profile, Resampling.nearest)
    ai_ok = np.isfinite(ai) & (ai > 0)
    masks = {
        "Global": footprint,
        "Water-limited": footprint & ai_ok & (ai < AI_THRESHOLD),
        "Energy-limited": footprint & ai_ok & (ai >= AI_THRESHOLD),
    }
    print(f"Footprint pixels: {footprint.sum():,}  "
          f"(water-limited {masks['Water-limited'].sum():,}, "
          f"energy-limited {masks['Energy-limited'].sum():,})")
    ...
    # Linear trends.
    print("\n=== 2001-2023 trends ===")
    for name in ("Global", "Water-limited", "Energy-limited"):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["year", "Global_gCm2",
                         "WaterLimited_gCm2", "EnergyLimited_gCm2"])
        for i, y in enumerate(YEARS):
            writer.writerow([y, round(series["Global"][i], 4),
                             round(series["Water-limited"][i], 4),
                             round(series["Energy-limited"][i], 4)])
